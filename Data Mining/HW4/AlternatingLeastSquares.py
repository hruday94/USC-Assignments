...
if __name__ == "__main__":
    ...
    inputMatrix = sc.textFile(sys.argv[1])
    NewMatrix = matrix(np.zeros((M,U))) #initializing the matrix
    
    elements = inputMatrix.map(lambda x:x.split(','))#extracting the values
    elements = elements.collect()
    
    for element in elements:
        NewMatrix[int(element[0])-1,int(element[1])-1] = int(element[2])
